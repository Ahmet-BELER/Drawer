...
import styles from "./flashlights.style"
...
const Flashlights = ({ navigation }) => {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchData = async () => {
    try {
      const response = await axios.get("https://m104e.wiremockapi.cloud/v1/flashlights", {
